...
void zen_game_game_update();
void zen_game_game_draw();
...
#define zen_game_MAX_ENTITIES 32
...
void* zen_game_entities[32];
int zen_game_entityIndex;
...
void zen_game_map_load()
{
	...
	int zen_game_x = 0;
	int zen_game_y = 0;
	for (int zen_game_i = 0; zen_game_LEVEL[zen_game_i] != '\0'; zen_game_i += 1)
	{
		if (zen_game_LEVEL[zen_game_i] == '\n')
		{
			zen_game_x = 0;
			zen_game_y += 1;
		}
		...
		{
			char8 zen_game_a = zen_game_LEVEL[zen_game_i];
			if (zen_game_a == '@')
			{
				zen_game_player.entity.x = zen_game_x;
				zen_game_player.entity.y = zen_game_y;
			}
			if (zen_game_a == 'c')
			{
				zen_game_Crate* crate = (zen_game_Crate*)malloc(sizeof(zen_game_Crate));
				zen_game_entities[zen_game_entityIndex] = crate;
				zen_game_entityIndex += 1;
				printf("%c", zen_game_a);
			}
		}
		zen_game_x += 1;
...
void zen_game_map_draw()
{
	...
	for (int i = 0; i < zen_game_entityIndex; i++)
	{
		zen_game_crate_draw(zen_game_entities[i]);
	}
...
void zen_game_map_update()
{
	for (int i = 0; i < zen_game_entityIndex; i++)
	{
		zen_game_Crate* crate = (zen_game_Crate*)zen_game_entities[i];
		zen_game_entity_update(&crate->entity);
	}
